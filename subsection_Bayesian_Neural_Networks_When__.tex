...
The entries of the weight matrices in $\mathcal{W}$ are \emph{a priori} distributed according to a Gaussian distribution with zero mean and variance equal to $\lamba^{-1}$. The prior distributions for $\gamma$ and $\lambda$ are non-informative Gamma distributions with shape and rate parameters equal to 6.
