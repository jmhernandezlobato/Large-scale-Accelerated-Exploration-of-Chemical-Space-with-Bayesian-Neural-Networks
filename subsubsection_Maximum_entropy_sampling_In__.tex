...
In the paradigm of probabilistic modeling of data, one can reduce error fast by obtaining data for the maximally informative molecules. For this, we follow the information-based approach described by \citeauthor{MacKay_1992}. The goal is to select the next data point so that it will on average shrink the most our current uncertainty on the optimal value for the network weights. This is equivalent to maximize the expected reduction in posterior entropy that is produced by adding data to the training set. This implies choosing the next data point $\mathbf{x}$ that maximizes
\begin{equation}
