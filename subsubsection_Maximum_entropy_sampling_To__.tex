...
The previous approach assumes that we are collecting data for one molecule at a time. However, in practice we may be able to perform a batch of experiments simultaneously. For example, by running computations in parallel in a computer cluster. With batch sizes larger than one, if we just select molecules as ranked by the variance of their predictive distributions, we run the risk of including in our batches molecules that are very similar to each other. To avoid this problem, we can avoid including in the current batch any new molecule that is too similar to the molecules already included. For example, by
