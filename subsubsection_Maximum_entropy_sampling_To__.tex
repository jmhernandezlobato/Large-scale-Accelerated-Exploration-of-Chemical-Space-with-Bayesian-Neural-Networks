...
The previous approach assumes that we are collecting data for one molecule at a time. However, in practice we may be able to perform multiple experiments simultaneously. For example, by running computations in parallel in a computer cluster. In this case, we would like to operate in the batch evaluation setting where multiple 
