...
\end{equation}
Note that for fixed $\mathbf{w}$ the predictions of the neural network are Gaussian with variance $\sigma^2$ independent of $\mathbf{x}$. This means that the entropy of $p(y| \mathbf{x},\mathcal{D})$ is independent of $\mathbf{x}$.
