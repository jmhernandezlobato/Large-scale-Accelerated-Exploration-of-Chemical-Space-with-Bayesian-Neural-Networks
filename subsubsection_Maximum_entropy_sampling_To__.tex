...
\end{equation}
The last term in~(\ref{eq:new_acquisition_function}) is constant because, for fixed $\mathbf{w}$, the predictions of the neural network are Gaussian with variance $\sigma^2$ independent of $\mathbf{x}$. Therefore, the entropy of $p(y| \mathbf{x},\mathcal{D})$
