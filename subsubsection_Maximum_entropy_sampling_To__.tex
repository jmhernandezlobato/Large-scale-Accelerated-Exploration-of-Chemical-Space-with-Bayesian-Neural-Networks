...
collect data in batches of multiple molecules by performing several experiments simultaneously. For example, by running computations in parallel in a computer cluster. In this case, if we just select molecules as ranked by the variance of their predictive distribution, we run the risk of including in our batch molecules that are too similar to each other, which is undesirable. To avoid this, we can exclude from the current batch any new molecule that is too similar to one of the already included molecules. For example, if the finguerprint-based Tanimoto distance with respect to one of the already included molecules is larger than 0.9. This is a simple but scalable approach that can be applied with large batch sizes. 
